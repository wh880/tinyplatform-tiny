...
public class ModelPlugin extends AbstractConfiguration implements Plugin {
	...
	private void insertView(EntityModel entityModel) {
		...
		try {
			DBOperator operator = beanManager.getDbOperator();
			if (views.size() > 0) {
				...
				for (int i = 0; i < views.size(); i++) {
					...
					setBeanProperty(viewBean, "menuName",
							entityModel.getTitle() + view.getTitle());
					setBeanProperty(viewBean, "description",
							view.getDescription());
		...
		if (operations.size() > 0) {
			...
			try {
				...
				for (int i = 0; i < operations.size(); i++) {
					...
					setBeanProperty(opBean, "functionName",
							entityModel.getTitle() + operation.getTitle());
					setBeanProperty(opBean, "description",
							operation.getDescription());
					setBeanProperty(opBean, "functionType",
							entityModel.getName());
					setBeanProperty(
							opBean,
							"functionUrl",
