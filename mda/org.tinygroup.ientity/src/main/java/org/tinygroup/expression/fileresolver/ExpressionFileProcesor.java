...
public class ExpressionFileProcesor extends AbstractFileProcessor {
    ...
    public void process() {
        ...
        for (FileObject fileObject : deleteList) {
            logger.logMessage(LogLevel.DEBUG, "正在移除表达式配置文件[{0}]", fileObject.getAbsolutePath());
            ...
            logger.logMessage(LogLevel.DEBUG, "移除表达式配置文件[{0}]结束", fileObject.getAbsolutePath());
        }
        for (FileObject fileObject : changeList) {
            logger.logMessage(LogLevel.DEBUG, "正在加载表达式配置文件[{0}]", fileObject.getAbsolutePath());
            ...
            ExpressionConfigs expressions = null;
            try {
                expressions = (ExpressionConfigs) stream.fromXML(fileObject.getInputStream());
                manager.addExpressions(expressions);
                caches.put(fileObject.getAbsolutePath(), expressions);
                logger.logMessage(LogLevel.DEBUG, "加载表达式配置文件[{0}]结束", fileObject.getAbsolutePath());
            } catch (IOException e) {
                logger.errorMessage("加载表达式配置文件[{0}]错误", e, fileObject.getAbsolutePath());
            }
        }
