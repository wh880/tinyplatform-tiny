...
public class SessionImpl implements HttpSession {
                    ...
                    if (log.isDebugEnabled()) {
                        log.debug(
                                ...
                                new Object[] { modelSessionID, new Date(model.getCreationTime()),
                                               ...
                                               getSessionWebContext().getSessionConfig().getForceExpirationPeriod() });
                    }
    ...
    public void commit(boolean commitHeaders) {
        String[] storeNames = webContext.getSessionConfig().getStores().getStoreNames();
            ...
            if (attr.isModified()) {
                ...
                if (isApplicableToCommit(store, commitHeaders)) {
                    ...
                    if (attrValue instanceof SessionModel) {
                        attrValue = webContext.getSessionConfig().getSessionModelEncoders()[0]
        ...
        if (mayNotBeCommitted != null) {
            for (Map.Entry<String, StringBuilder> entry : mayNotBeCommitted.entrySet()) {
                ...
                SessionStore store = webContext.getSessionConfig().getStores().getStore(storeName);
        ...
        // 则不提交了，直接退出。
        if (!modified && !cleared && !webContext.getSessionConfig().isKeepInTouch()) {
            return;
        }
    ...
    private class HttpSessionInternal implements HttpSession {
        ...
        private Object decodeSessionModel(Object value) {
            ...
            SessionModelEncoder[] encoders = webContext.getSessionConfig().getSessionModelEncoders();
