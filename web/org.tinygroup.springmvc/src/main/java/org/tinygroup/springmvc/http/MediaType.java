...
public class MediaType implements Comparable<MediaType> {
	 ...
	 * <p>
	 * The {@linkplain #getSubtype() subtype} is set to {@code &#42;},
	 * parameters empty.
	 * 
	 * @param type
	 *            the primary type
	...
	public MediaType(String type, String subtype, Charset charset) {
		this(type, subtype, Collections.singletonMap(PARAM_CHARSET,
				charset.name()));
	}
	...
	public MediaType(String type, String subtype, Map<String, String> parameters) {
		...
		if (!CollectionUtils.isEmpty(parameters)) {
			Map<String, String> m = new LinkedCaseInsensitiveMap<String>(
					parameters.size(), Locale.ENGLISH);
	 ...
	 * character {@code &#42;} or the wildcard character followed by a sufiix
	 * (e.g. {@code &#42;+xml}), or not.
	 * 
	...
	/**
	 * Indicates whether this media type is concrete, i.e. whether neither the
	 * type or subtype is a wildcard character {@code &#42;}.
	 * 
	...
	public double getQualityValue() {
		...
		return (qualityFactory != null ? Double
				.parseDouble(unquote(qualityFactory)) : 1D);
	}
		...
		if (this.isWildcardType()) {
			...
			if (this.isWildcardSubtype()) {
				...
				if (thisPlusIdx == -1) {
					...
					if (otherPlusIdx != -1) {
						String thisSubtypeNoSuffix = this.subtype.substring(0,
								thisPlusIdx);
						String thisSubtypeSuffix = this.subtype
								.substring(thisPlusIdx + 1);
						String otherSubtypeSuffix = other.subtype
		...
		if (isWildcardType() || other.isWildcardType()) {
			...
			if (this.isWildcardSubtype() || other.isWildcardSubtype()) {
				...
				if (thisPlusIdx == -1 && otherPlusIdx == -1) {
					...
					String thisSubtypeNoSuffix = this.subtype.substring(0,
							thisPlusIdx);
					String otherSubtypeNoSuffix = other.subtype.substring(0,
							otherPlusIdx);
					...
					if (thisSubtypeSuffix.equals(otherSubtypeSuffix)
							&& (WILDCARD_TYPE.equals(thisSubtypeNoSuffix) || WILDCARD_TYPE
									.equals(otherSubtypeNoSuffix))) {
						return true;
					}
				}
			}
	...
	public MediaType copyQualityValue(MediaType mediaType) {
		...
		params.put(PARAM_QUALITY_FACTOR,
				mediaType.parameters.get(PARAM_QUALITY_FACTOR));
	...
	public int compareTo(MediaType other) {
		...
		TreeSet<String> thisAttributes = new TreeSet<String>(
				String.CASE_INSENSITIVE_ORDER);
		thisAttributes.addAll(this.parameters.keySet());
		TreeSet<String> otherAttributes = new TreeSet<String>(
				String.CASE_INSENSITIVE_ORDER);
	...
	public boolean equals(Object other) {
		...
		return (this.type.equalsIgnoreCase(otherType.type)
				&& this.subtype.equalsIgnoreCase(otherType.subtype) && this.parameters
	...
	public static String toString(Collection<MediaType> mediaTypes) {
		StringBuilder builder = new StringBuilder();
		for (Iterator<MediaType> iterator = mediaTypes.iterator(); iterator
				.hasNext();) {
	...
	public static void sortBySpecificityAndQuality(List<MediaType> mediaTypes) {
		...
		if (mediaTypes.size() > 1) {
			Collections.sort(mediaTypes, new CompoundComparator(
					new Comparator[] { MediaType.SPECIFICITY_COMPARATOR,
			...
			if (mediaType1.isWildcardType() && !mediaType2.isWildcardType()) { // */*
				...
				if (mediaType1.isWildcardSubtype()
					...
					return -1;
						...
						mediaType2.getSubtype())) { // audio/basic == audio/wave
					return 0;
					...
					if (qualityComparison != 0) {
						...
						return (paramsSize2 < paramsSize1 ? -1
								: (paramsSize2 == paramsSize1 ? 0 : 1)); // audio/basic;level=1
																			// <
																			// audio/basic
					}
				}
