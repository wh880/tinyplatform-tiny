...
public class RestfulStyleSubstitutionHandler implements
	...
	public void postSubstitution(RewriteSubstitutionContext context) {
		...
		RestfulContext restfulContext = urlRestfulManager
				.getUrlMappingWithRequet(orignalPath, httpMethod, requestAccept);
		...
		if (!CollectionUtil.isEmpty(variableMap)) {
			for (String key : variableMap.keySet()) {
				...
				if (!StringUtil.isBlank(value)) {
					String[] paramterValues = value.split(",");
					if (paramterValues.length > 1) {
						parameterParser.setObjects(key, paramterValues);
					} else {
