...
public class FileResolverUtil {
	private static Logger logger = LoggerFactory.getLogger(FileResolverUtil.class);
	public List<String> getClassPath(FileResolver resolver) {
	...
	boolean isInclude(FileObject fileObject, FileResolver resolver) {
	...
	public List<String> getWebLibJars(FileResolver resolver) throws Exception {
	...
	private void addJarFile(List<String> classPaths, String path) {
        logger.logMessage(LogLevel.INFO, "扫描到jar文件<{}>。", path);
        classPaths.add(path);
    }

	public List<String> getWebClasses() {
