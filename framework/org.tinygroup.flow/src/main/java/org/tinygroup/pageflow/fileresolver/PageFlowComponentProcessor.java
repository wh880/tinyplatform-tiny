...
public class PageFlowComponentProcessor extends AbstractFileProcessor {
    ...
    public void process() {
        ...
        for (FileObject fileObject : changeList) {
            logger.logMessage(LogLevel.INFO, "正在读取页面组件pagefc文件[{0}]", fileObject.getAbsolutePath());
            ComponentDefines oldComponents = (ComponentDefines) caches.get(fileObject.getAbsolutePath());
            ...
            try {
                ...
                logger.logMessage(LogLevel.INFO, "读取页面组件pagefc文件[{0}]结束", fileObject.getAbsolutePath());
            } catch (IOException e) {
                logger.errorMessage("读取页面组件pagefc文件[{0}]错误", e, fileObject.getAbsolutePath());
            }
        }
