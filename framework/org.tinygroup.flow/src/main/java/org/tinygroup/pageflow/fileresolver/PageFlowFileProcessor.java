...
public class PageFlowFileProcessor extends AbstractFileProcessor {
    ...
    public void process() {
        ...
        for (FileObject fileObject : deleteList) {
            logger.logMessage(LogLevel.INFO, "正在删除页面流程flow文件[{0}]", fileObject.getAbsolutePath());
            Flow flow = (Flow) caches.get(fileObject.getAbsolutePath());
            if (flow != null) {
                flowExecutor.removeFlow(flow);
                caches.remove(fileObject.getAbsolutePath());
            }
            logger.logMessage(LogLevel.INFO, "删除页面流程flow文件[{0}]结束", fileObject.getAbsolutePath());
        }
        for (FileObject fileObject : fileObjects) {
            logger.logMessage(LogLevel.INFO, "正在读取页面流程pageflow文件[{0}]", fileObject.getAbsolutePath());
            ...
            try {
                ...
                logger.logMessage(LogLevel.INFO, "读取页面流程pageflow文件[{0}]结束", fileObject.getAbsolutePath());
            } catch (IOException e) {
                logger.errorMessage("读取页面流程pageflow文件[{0}]错误", e, fileObject.getAbsolutePath());
            }
        }
