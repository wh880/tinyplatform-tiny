...
public class PageFlowComponentRemoveProcessor extends AbstractFileProcessor {
    ...
    public void process() {
        ...
        for (FileObject fileObject : fileObjects) {
            logger.logMessage(LogLevel.INFO, "正在删除页面组件pagefc文件[{0}]", fileObject.getAbsolutePath());
            ...
            try {
                ...
                logger.logMessage(LogLevel.INFO, "删除页面组件pagefc文件[{0}]结束", fileObject.getAbsolutePath());
            } catch (IOException e) {
                logger.errorMessage("删除页面组件pagefc文件[{0}]错误", e, fileObject.getAbsolutePath());
            }
        }
