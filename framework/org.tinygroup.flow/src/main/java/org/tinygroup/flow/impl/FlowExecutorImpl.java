...
public class FlowExecutorImpl implements FlowExecutor {
	...
	public void execute(String flowId, String nodeId, Context context) {
		...
		if (executorNodeId == null
				|| executorNodeId.equals(flow.getBeginNodeId())) {
			executorNodeId = flow.getBeginNodeId();
		}
	...
	public void execute(Flow flow, Node node, Context context) {
		...
		try {
			...
			if (component != null) {
				...
				if (!nodeId.equals(flow.getEndNodeId())) { // 如果当前节点不是最终节点
					componentInstance.execute(flowContext);
				}
			}
			if (nodeId != null && !nodeId.equals(flow.getEndNodeId())) {
				...
				if (nextNodeId == null) {
					String flowDefaultNodeId = flow.getDefaultNodeId();
					if (flowDefaultNodeId != null
							&& !flowDefaultNodeId.equals(Flow.END_ID)) {
						nextNodeId = flowDefaultNodeId;
					}
				}
				if (nextNodeId == null) {
	...
	private void executeNextNode(Flow flow, Context context, String nextNodeId) {
		if (nextNodeId != null && !nextNodeId.equals(flow.getEndNodeId())) {
	...
	public void addFlow(Flow flow) {
		...
		if (flow.getId() != null) {
			logger.logMessage(LogLevel.INFO, "添加flow:[id:{0},version:{1}]",
					flow.getId(), flow.getVersion());
			flowIdMap.put(flow.getId(), flow);
		}
		if (flow.getName() != null) {
			logger.logMessage(LogLevel.INFO, "添加flow:[Name:{0},version:{1}]",
	...
	public void removeFlow(Flow flow) {
		logger.logMessage(LogLevel.INFO, "移除flow:[id:{0},version:{1}]",
				flow.getId(), flow.getVersion());
	...
	public void execute(String flowId, String nodeId, String version,
			Context context) {
		Flow flow = flowIdVersionMap.get(getKey(flowId, version));
		if (nodeId == null) {
			nodeId = flow.getBeginNodeId();
		}
		if (flow != null) {
	...
	public Flow getFlow(String flowId, String version) {
		...
		return flowIdVersionMap.get(getKey(flowId, version));
	}
