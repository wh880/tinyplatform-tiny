...
public class BundleManagerDefault implements BundleManager {
	private static Logger logger = LoggerFactory.getLogger(BundleManagerDefault.class);
    ...
    private void startBundle(BundleContext bundleContext, BundleDefine bundleDefine, String bundle){
    	String dependencies = bundleDefine.getDependencies();
    	String[] dependens = dependencies.split(",") ; //获取所依赖的bundle项
    	for(String dependen:dependens){ //启动所有的依赖项
    		logger.logMessage(LogLevel.DEBUG, "开始启动Bundle:{0}所依赖Bundle:{1}",bundle,dependen);
    		start(bundleContext,dependen);
    		logger.logMessage(LogLevel.DEBUG, "启动Bundle:{0}所依赖Bundle:{1}完毕",bundle,dependen);
    	}
    	...
    	if(activatorBean!=null&&!"".equals(activatorBean)){
    		...
    		try {
				...
				logger.errorMessage("启动Bundle:{0}的Activator:{1}时出错",e,bundle,activatorBean);
			}
    	}
    ...
    private void stopBundle(BundleContext bundleContext, BundleDefine bundleDefine, String bundle){
    	...
    	for(String dependenBy:dependencyByList){ //启动所有的依赖项
    		logger.logMessage(LogLevel.DEBUG, "开始停止依赖Bundle:{0}的Bundle:{1}",bundle,dependenBy);
    		stop(bundleContext,dependenBy);
    		logger.logMessage(LogLevel.DEBUG, "停止依赖Bundle:{0}的Bundle:{1}完毕",bundle,dependenBy);
    	}
    	...
    	if(activatorBean!=null&&!"".equals(activatorBean)){
    		...
    		try {
				...
				logger.errorMessage("停止Bundle:{0}的Activator:{1}时出错",e,bundle,activatorBean);
			}
    	}
     ...
     * @param events 需要处理的事件列表
     * @param bundleContext
     * @param bundleDefine 
     */
    private void processEvents(List<BundleEvent> events,BundleContext bundleContext,BundleDefine bundleDefine){
