...
public class CEPCoreCentralImpl extends AbstractCEPCoreOp implements
	...
	private static Logger logger = LoggerFactory
			.getLogger(CEPCoreCentralImpl.class);
	...
	public void registeNode(List<Node> nodes, Node sourceNode) {
		for (Node node : nodes) {
			registeCepNode(node,sourceNode);
		}
	}
	...
	public void registeNode(Node node, Node sourceNode) {
		...
		if (Node.CEP_NODE.equals(node.getType())) {
			...
			registeCepCentral(node,sourceNode);
		}
	}

	private synchronized void registeCepNode(Node node, Node sourceNode) {

		logger.logMessage(LogLevel.INFO, "注册CEP节点{0}", node.toString());
		registeCepNodeToExist(node,sourceNode);
		registeExistToCepNode(node,sourceNode);
		logger.logMessage(LogLevel.INFO, "注册CEP节点{0}完成", node.toString());
		
		
	}
	
	private void registeCepNodeToExist(Node node, Node sourceNode){
		...
		distributeCepNode(node,sourceNode, REG_NODE_SERVICE, c);
		logger.logMessage(LogLevel.INFO, "推送该节点至服务中心已有节点列表完成");
	}
	private void registeExistToCepNode(Node node, Node sourceNode){
	...
	protected Event getEvent(String serviceId, Context context){
		Event e = super.getEvent(serviceId, context);
		e.setPriority(SYS_PRIORITY);
		return e;
	}
	private void distributeCepNode(Node node,Node sourceNode, String serviceId, Context c) {
		Event event = getEvent(serviceId, c);
		String nodeKey = CEPCoreUtil.getNodeKey(node);
		String sourceKey = CEPCoreUtil.getNodeKey(sourceNode);
		for (String key : cepNodesMap.keySet()) {
			if (!key.equals(nodeKey)&&!key.equals(sourceKey)) {
				Node cep = cepNodesMap.get(key);
				try {
					logger.logMessage(LogLevel.INFO,"开始向CEP:{0}发送请求[id:{1}]",
							cep.toString(), serviceId);
					remoteprocess(event, cep);
					logger.logMessage(LogLevel.INFO,"向CEP:{0}发送请求[id:{1}]完成",
							cep.toString(), serviceId);
				} catch (Exception e) {
	...
	private synchronized void registeCepCentral(Node node, Node sourceNode) {
	...
	public void unregisteNode(Node node,Node sourceNode) {
		if (Node.CEP_NODE.equals(node.getType())) {
			unregisteCepNode(node,sourceNode);
		} else if (Node.CENTRAL_NODE.equals(node.getType())) {
			unregisteCepCentral(node,sourceNode);
		}
		removeConnect(node);

	}

	private synchronized void unregisteCepCentral(Node node,Node sourceNode) {
		//centralNodesMap.remove(node.getNodeName());
	}

	private synchronized void unregisteCepNode(Node node,Node sourceNode) {
		logger.logMessage(LogLevel.INFO, "节点{0}注销CEP节点{1}", sourceNode.toString(),node.toString());
		String nodeKey = CEPCoreUtil.getNodeKey(node);
		...
		c.put("node", cepNode);
		c.put("sourceNode", getNode());
		distributeCepNode(cepNode,sourceNode, UNREG_NODE_SERVICE, c);
		logger.logMessage(LogLevel.INFO, "节点{0}注销CEP节点{1}完成", sourceNode.toString(), node.toString());
	}
