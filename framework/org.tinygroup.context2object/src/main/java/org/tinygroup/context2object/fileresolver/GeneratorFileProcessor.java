...
public class GeneratorFileProcessor extends AbstractFileProcessor {
    private static Logger logger = LoggerFactory.getLogger(GeneratorFileProcessor.class);
    ...
    public void process() {
        ClassNameObjectGenerator generator = SpringUtil.getBean(CLASSNAME_OBJECT_GENERATOR_BEAN);
        ...
        for (FileObject file : changeList) {
            logger.logMessage(LogLevel.INFO, "开始读取generator配置文件:{0}", file.getFileName());
            ...
            try {
                config = (GeneratorConfig) stream.fromXML(file.getInputStream());
                deal(config, generator);
                caches.put(file.getAbsolutePath(), config);
                logger.logMessage(LogLevel.INFO, "读取generator配置文件:{0}完成", file.getFileName());
            } catch (IOException e) {
                logger.errorMessage("读取generator配置文件:{0}错误", e, file.getFileName());
            }

        }
    }

    private void removeConfig(GeneratorConfig config, ClassNameObjectGenerator generator) {
        for (GeneratorConfigItem item : config.getTypeConverters()) {
            logger.logMessage(LogLevel.INFO, "处理TypeConverter,beanName:{0},className:{1}", item.getBeanName(), item.getClassName());
        ...
        for (GeneratorConfigItem item : config.getTypeCreators()) {
            logger.logMessage(LogLevel.INFO, "处理TypeCreator,beanName:{0},className:{1}", item.getBeanName(), item.getClassName());
    ...
    private Object deal(GeneratorConfigItem configItem) {
        ...
        if (beanName == null || "".equals(beanName)) {
            try {
                ...
                logger.errorMessage("generator配置文件加载时，创建类:{0}出错", e, configItem.getClassName());
