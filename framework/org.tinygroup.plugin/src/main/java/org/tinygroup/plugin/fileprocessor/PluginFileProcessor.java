...
public class PluginFileProcessor extends AbstractFileProcessor {
    ...
    public void process() {
        ...
        for (FileObject file : deleteList) {
            logger.logMessage(LogLevel.INFO, "移除plugin配置文件:{0}", file.getFileName());
            ...
            logger.logMessage(LogLevel.INFO, "移除plugin配置文件:{0}完成", file.getFileName());
        }
        for (FileObject file : changeList) {
            logger.logMessage(LogLevel.INFO, "开始读取plugin配置文件:{0}", file.getFileName());
            PluginConfigs oldConfigs = (PluginConfigs) caches.get(file.getAbsolutePath());
            if (oldConfigs != null) {
                manager.removePlugin(oldConfigs);
            }
            PluginConfigs configs = null;
            try {
                configs = (PluginConfigs) stream.fromXML(file.getInputStream());
                manager.addPlugin(configs);
                caches.put(file.getAbsolutePath(), configs);
                logger.logMessage(LogLevel.INFO, "读取plugin配置文件:{0}完成", file.getFileName());
            } catch (IOException e) {
                logger.errorMessage("读取plugin配置文件:{0}错误", e, file.getFileName());
            }
        }
    }
