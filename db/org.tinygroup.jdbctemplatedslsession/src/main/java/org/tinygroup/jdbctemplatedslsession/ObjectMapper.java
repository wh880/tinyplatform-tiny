...
public class ObjectMapper {
	...
	public <T> T assemble(boolean autoGeneratedKeys,
			...
			if (pd != null) {
				try {
					...
					if (isPrimaryColumn(column, tableMetaData)) {
						...
						value = getColumnValue(column,tableMetaData, newContext);
					}
		...
		if (!autoGeneratedKeys) {// 执行数据库插入操作
			...
			dslSession.getJdbcTemplate().update(insertBody.toString(),
	 ...
	 * @return
	 */
	private Object getColumnValue(String columnName,TableMetaData tableMetaData, InsertContext insertContext) {
	 ...
	 * @return
	 */
	private boolean isPrimaryColumn(String columnName, TableMetaData tableMetaData) {
