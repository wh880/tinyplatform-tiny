...
public class DialectFunctionlFileResolver extends AbstractFileProcessor {
    ...
    public void process() {
        DialectFunctionProcessor functionProcessor = SpringUtil.getBean(DataBaseUtil.FUNCTION_BEAN);
        ...
        for (FileObject fileObject : changeList) {
            logger.logMessage(LogLevel.INFO, "正在加载function文件[{0}]", fileObject.getAbsolutePath());
            ...
            DialectFunctions functions = null;
            try {
                functions = (DialectFunctions) stream.fromXML(fileObject.getInputStream());
                functionProcessor.addDialectFunctions(functions);
                caches.put(fileObject.getAbsolutePath(), functions);
                logger.logMessage(LogLevel.INFO, "加载function文件[{0}]结束", fileObject.getAbsolutePath());
            } catch (IOException e) {
                logger.errorMessage("加载function文件[{0}]错误", e, fileObject.getAbsolutePath());
            }
        }
    }
