...
public class ViewFileResolver extends AbstractFileProcessor {
    ...
    public void process() {
        ViewProcessor viewProcessor = SpringUtil.getBean(DataBaseUtil.VIEW_BEAN);
        XStream stream = XStreamFactory.getXStream(DataBaseUtil.DATABASE_XSTREAM);
        for (FileObject fileObject : deleteList) {
            logger.logMessage(LogLevel.INFO, "正在移除view文件[{0}]", fileObject.getAbsolutePath());
            Views views = (Views) caches.get(fileObject.getAbsolutePath());
            if (views != null) {
                viewProcessor.removeViews(views);
                caches.remove(fileObject.getAbsolutePath());
            }
            logger.logMessage(LogLevel.INFO, "移除view文件[{0}]结束", fileObject.getAbsolutePath());
        }
        for (FileObject fileObject : changeList) {
            logger.logMessage(LogLevel.INFO, "正在加载view文件[{0}]", fileObject.getAbsolutePath());
            ...
            Views views = null;
            try {
                views = (Views) stream.fromXML(fileObject.getInputStream());
                viewProcessor.addViews(views);
                caches.put(fileObject.getAbsolutePath(), views);
                logger.logMessage(LogLevel.INFO, "加载view文件[{0}]结束", fileObject.getAbsolutePath());
            } catch (IOException e) {
                logger.errorMessage("加载view文件[{0}]错误", e, fileObject.getAbsolutePath());
            }
        }
    }
