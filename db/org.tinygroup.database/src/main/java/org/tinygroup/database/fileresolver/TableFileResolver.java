...
public class TableFileResolver extends AbstractFileProcessor {
    ...
    public void process() {
        TableProcessor tableProcessor = SpringUtil.getBean(DataBaseUtil.TABLEPROCESSOR_BEAN);
        ...
        for (FileObject fileObject : changeList) {
            logger.logMessage(LogLevel.INFO, "正在加载table文件[{0}]", fileObject.getAbsolutePath());
            ...
            Tables tables = null;
            try {
                tables = (Tables) stream.fromXML(fileObject.getInputStream());
                tableProcessor.addTables(tables);
                caches.put(fileObject.getAbsolutePath(), tables);
                logger.logMessage(LogLevel.INFO, "加载table文件[{0}]结束", fileObject.getAbsolutePath());
            } catch (IOException e) {
                logger.errorMessage("加载table文件[{0}]错误", e, fileObject.getAbsolutePath());
            }
        }
    }
