...
public class RelationFileProcessor extends AbstractFileProcessor {
    ...
    public void process() {
        ...
        XStream stream = XStreamFactory.getXStream(BeanOperatorManager.XSTEAM_PACKAGE_NAME);
        for (FileObject fileObject : deleteList) {
            logger.logMessage(LogLevel.INFO, "正在移除数据库关联配置文件[{0}]", fileObject.getAbsolutePath());
            Relations relations = (Relations) caches.get(fileObject.getAbsolutePath());
            if (relations != null) {
                manager.removeRelationConfigs(relations);
                caches.remove(fileObject.getAbsolutePath());
            }
            logger.logMessage(LogLevel.INFO, "移除数据库关联配置文件[{0}]结束", fileObject.getAbsolutePath());
        }
        for (FileObject fileObject : changeList) {
            logger.logMessage(LogLevel.INFO, "正在加载数据库关联配置文件[{0}]", fileObject.getAbsolutePath());
            ...
            try {
                relations = (Relations) stream.fromXML(fileObject.getInputStream());
                manager.addRelationConfigs(relations);
                caches.put(fileObject.getAbsolutePath(), relations);
                logger.logMessage(LogLevel.INFO, "加载数据库关联配置文件[{0}]结束", fileObject.getAbsolutePath());
            } catch (IOException e) {
                logger.errorMessage("加载数据库关联配置文件[{0}]错误", e, fileObject.getAbsolutePath());
            }
        }
